...
function findSimilar(root:SearchNode, lang:string, category:string, chapter: string, doc:string) {
    ...
    for (const langNode of root.children??[]) {
        const rank1 = like(langNode.title, title(lang)) ? 1 : 0;

        if (like(langNode.title, title(lang)) && !category) {
            similar.push({...langNode, rank: rank1 + 1});
            break;
        }

        for (const categoryNode of langNode.children??[]) {
            const rank2 = rank1 + (like(categoryNode.title, title(category)) ? 2 : 0);

            if (like(categoryNode.title, title(category)) && !chapter) {
                similar.push({...categoryNode, rank: rank2 + 2});
                break;
            }

            for (const chapterNode of categoryNode.children??[]) {
                const rank3 = rank2 + (like(chapterNode.title, title(chapter)) ? 3 : 0);
                ...
                for (const docNode of chapterNode.children??[]) {
                    ...
                    similar.push({...docNode, text: `${categoryNode.title} -> ${chapterNode.title} -> ${docNode.title}`, rank: rank3 + 4});
                }
            }
        }
    }
...
export default function NotFound(props:NotFoundProps) {
    ...
    return (
        <MainPage lang={lang} depth={1} disableDiscussion>
            <Paper sx={{flex:1}}>
                <Typography variant="h1" align="center" sx={{fontSize:'clamp(1.25rem, 6vw, 2.5rem)', fontWeight:'bolder', marginBlock:'1rem'}}>
                    {
                        similar[0]?.rank === 10 ?
                        <>Waiting...</> :
                        <>404 Not Found</>
